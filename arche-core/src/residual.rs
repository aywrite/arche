...
//! The residual is the reference's answer less the claim.
...
/// The key a shortcut's answer at this node is sampled by. A counter over
/// the stream would move the membership with any change to the tree, which
/// reads as a shift in the distribution.
...
pub struct Sample {
    /// The fifty move counter travels in the fen; the path does not (see
    /// `replay`).
    pub fen: String,
    ...
    pub claimed: Score,
    /// The beta the shortcut cleared, which says whether the cutoff was
    /// earned.
    pub beta: Score,
    /// Static evaluation less beta. Wider than a `Score` because the
    /// difference of two is not one.
    pub eval_beta: i32,
    pub window: Window,
    /// The fifty move counter, pulled out of the fen so rows filter on it.
    pub halfmove: usize,
}

/// One sample with the reference's answer beside the shortcut's.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct Row {
    pub kind: Shortcut,
    pub depth: u8,
...
impl Row {
    /// The reference's answer less the claim. Negative is the shortcut
    /// claiming more than the search found.
    pub fn delta(&self) -> i32 {
        i32::from(self.reference) - i32::from(self.claimed)
    }

    /// Whether the reference's answer fell below the beta the shortcut
    /// cleared. Strictly below, since an answer equal to beta is a fail high
    /// the shortcut was entitled to take.
    ...
    /// Whether the claim is strictly above the reference's answer.
    ///
    ...
    /// when no move does better, and the ceiling stored is then too low. On
    /// a shadow row nothing was handed up, and the label says what the
    /// margin would have overstated had it fired.
    ///
    /// A mate reference is labelled like any other row, since an eval claim
    /// is above every mated score and below every mating score whatever the
    /// distance.
    ...
    /// Whether the reference answered with a forced mate. Such a row's delta
    /// is not a number of pawns (and its mate distance counts from the
    /// replay's root), so only its two labels are read.
    pub fn reference_is_mate(&self) -> bool {
        crate::value::is_mate(self.reference)
    }
}

/// The residuals of one kind at one depth. Not pooled: the margin grows with
/// the depth left and the depths are reached in very different numbers, so
/// a pooled rate is the shallowest depth's.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Summary {
    pub kind: Shortcut,
    pub depth: u8,
    /// Every row of the pair, mates included, which the rates are shares of.
    pub count: usize,
    pub crossed: usize,
    pub overstated: usize,
    /// Left out of the percentiles.
    pub mates: usize,
    /// None when every row of the pair is a mate.
...
impl Summary {
    /// A percentage. The count is never zero, since a pair with no rows has
    /// no summary.
...
pub struct Report {
    ...
    pub positions: usize,
    /// Every node the shortcuts answered, kept or not: the denominator.
    pub events: u64,
    pub overflowed: u64,
    /// Samples whose fen did not parse. A position with no move to make is
    /// not one of these: it has a value by rule and gets a row.
...
/// Small because each record is a shallow search that would not fill a
/// larger table.
pub const REPLAY_TABLE_BYTES: usize = 4 * 1024 * 1024;

/// Record, then replay, never both at once: re-searching during the
/// recording would write reference entries into the measured search's
/// table. `suite` is for the header alone.
pub fn run(
    positions: &[Position],
    suite: Option<&str>,
    depth: u8,
...
/// The engine a replay asks, here and in the reduction ledger: the
/// reference, since a shortcut cannot judge what it cost.
...
/// nothing when the fen does not parse. The table is cleared first: a
/// reference value that depends on what the replay searched before it is
...
pub(crate) fn reference_answer(engine: &mut AlphaBeta, fen: &str, depth: u8) -> Option<Score> {
    ...
    Some(match outcome {
        ...
        // no move to make: scored as the search scores it a ply down. A mate
        // on the hundredth half move is still a mate, and everything else is
        // a draw. A real run only brings a stalemate here, since a sampled
        // node is never in check and never past the counter
...
/// What the reference says about each sampled position, and how many it
/// could not read.
///
/// A fen carries the fifty move counter and not the path, so the replay
/// cannot see a repetition that needs moves made before the node. A
/// residual from deep in a shuffle is read with that in mind.
...
/// Nearest rank, so the median of an even count is the lower of the middle
/// pair and every figure printed is a residual that happened.
...
impl Report {
    pub fn summary(&self, kind: Shortcut, depth: u8) -> Option<Summary> {
        ...
        // at a small count a handful of mates would own every percentile
        // above the median
...
/// A header, a row a sample, and a summary line for each kind at each depth.
///
...
/// so the one field that can hold spaces holds the rest of the line.
///
/// The header always states the events: a run that recorded no crossing
/// has measured nothing until the reader knows how many chances it had.
///
/// Not yet columns, each waiting on plumbing outside this module: whether
/// the side to move is improving, the table entry's bound and depth at the
...
mod tests {
    ...
    /// The delta's sign, pinned against claims made up to be wrong in a
    /// known direction.
    #[test]
    ...
    /// The second sample is mated on the hundredth half move, which is a
    /// mate and not the draw the counter would claim.
    #[test]
    ...
    /// The one terminal position a real run reaches: a claim above beta on a
    /// side that has no move. Scored, not dropped.
    #[test]
    ...
    /// The second sample is in check with a square to run to, so check alone
    ...
    /// whose keys fall in the first seventh of the range. Asked of the
    /// recording alone, which costs no replay.
    #[test]
    ...
    /// The reference value is a real one, with beta made up either side.
    #[test]
    ...
    /// The deeper record leaves entries the shallower one would be answered
    /// from on a warm table. The first assertion is that the two depths
    /// disagree, without which the rest would pass on a table never cleared;
    /// the last two pin the depth as well. Which depths disagree depends on
    /// the evaluation (five and two stopped disagreeing at the 2026-09-13
    /// mobility refit), so a failure of the first assertion means picking
    /// another pair.
    #[test]
    ...
    /// A replay searching the default would print every crossing rate as a
    /// shortcut agreeing with itself.
    #[test]
    ...
    fn a_row_reads_left_to_right_with_the_fen_last() {
        let report = Report {
            ...
            rows: vec![Row {
                ...
                window: Window::Zero,
                halfmove: 12,
                // above the reference, so the row reads crossed
                beta: 180,
                eval_beta: 140,
                claimed: 200,
                reference: 150,
