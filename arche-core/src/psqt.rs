...
// 3,372,298 quiet rows extracted by `arche terms` at 54d85b9, with every weight
// but material fitted at once: these tables and the four leaf terms. The ridge
// is 1e-7 toward the weights these replace. Every table but the king's
// midgame one is folded left to right, so a square and its mirror read one
// weight; castling is not mirrored, so the king's midgame table is not. Five
// fold cross validation with whole pairs of games held out scores 0.083124 at
// the weights these replace and 0.082291 at these, a paired difference of
...
//
// Material was held at its shipped values because
...
// midgame twin (from 1.6 for the queen to 3.2 for the knight), against nineteen for the
// pawn and forty three for the king, and the difference is spread over the
// board rather than placed near the enemy king or on the seventh, where
// endgame piece placement is supposed to diverge. On this corpus the
