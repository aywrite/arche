...
pub const REDUCTIONS: Command = Command {
    ...
    summary: &[
        ...
        "reduced scouts, and ask a full depth search what each",
        "reduction would have thrown away",
    ],
};
...
/// keeps.
///
/// The suite is a setting here and on the reductions argument, and not on
/// the census, the way the taint policy is a setting here alone. What wants
/// it is a threshold chosen off these rows: a rule fitted on the bench's
/// positions and then read back on the same positions has checked nothing,
/// so the fit and the check are given separate files.
...
/// with and replays it with the reference, so there is no policy to
/// choose.
///
/// The suite is a setting here on the residuals argument's reason: a
/// reduction chosen off these rows and then read back on the same
...
mod tests {
    ...
    /// residuals argument's terms, and a file that is no suite is named
