...
- **eval/**: What a position scores: a file per leaf term, the pair term and
  ...
  - **factors.rs**: The pair term, a factorization machine over the piece
    ...
    `factors16.rs`, generated by a fitting program outside the repository;
    the `machine-test` feature swaps in a seeded rank 8 table so the tests
    check the term on a table no fit chose.
...
- **tune.rs**: What a position's evaluation is made of. The evaluation is
  ...
  position's side of the dot product, one coefficient per weight it touches.
  `reconstruct` folds a row back against the live tables and has to give the
  evaluation exactly. Driven by the `terms` argument, and read by
  `scripts/tune.py`.
