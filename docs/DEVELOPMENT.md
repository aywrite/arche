...
depth, the late move pruning that drops a late quiet whose place has
passed a higher floor of the same kind without searching it at all, the
reduction table those two amounts are read off, the killers and history
table the quiet moves are ordered by, and the aspiration window the
deepening loop opens each iteration at.
