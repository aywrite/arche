...
# Termination as fastchess spells it. abandoned covers two endings that are
# worth telling apart, and the reason tells them apart, so it is split into
# disconnect and stall by kind() below and appears here only for the game whose
...
FAULTS = ("time forfeit", "disconnect", "stall", "abandoned", "illegal move")
...
def kind(termination: str, reason: str) -> str:
    ...
    stands for. An engine that failed before its first move of its own leaves
    the game ending on a book move, whose comment fastchess writes as `book`
    with no reason after it, so that game stays abandoned: a fault, but not
    one the pgn says the kind of."""
    if termination == "abandoned":
        if "connection stalls" in reason:
            return "stall"
        if "disconnects" in reason:
            return "disconnect"
        return "abandoned"
