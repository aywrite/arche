...
# would move under the run that names it, and the manifest's book_sha256
# would record the change with nothing failing. The match tools cache key is
...
# default of both workflows. Eight moves of a real game apiece, balanced by
# construction, so a side does not start an opening ahead.
...
# Unbalanced on purpose. Under -repeat both engines play both colours of an
# opening, so an unbalanced book does not bias who wins; it raises how often a
# game is decided.
...
# Asked of the list rather than of the table: an associative array reads @ and
# * as every key, and the book arrives here from a text box.
known() {
    local book
    for book in $(list); do
        if [ "$book" = "$1" ]; then
...
# A pgn opening is a game, and every game carries an Event tag; an epd opening
# is a line. The file is counted rather than the block's figure printed,
# because a cache hit hands a run a file the fetch never checked.
count() {
...
fetch() {
    ...
    known "$book"
    local file=${FILE[$book]}
    # a trap rather than a last line, since under set -e a failed fetch has no
    # other way out. WORK is not local because the trap runs after this
    ...
    # before the move, so a run that fetched something else has nothing to play
