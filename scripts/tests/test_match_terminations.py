...
class TestKind:
    ...
    def test_abandoned_with_no_reason_stays_abandoned(self):
        # an engine that failed before its first move of its own leaves the
        # game ending on a book move, and fastchess writes no reason on those
        assert match_terminations.kind("abandoned", "book") == "abandoned"
