...
fn the_move_a_swap_answers_with_opens_the_last_line_said() {
    ...
    // The swap is at depth seven here, which finishes at 176,633 nodes, and
    // the depth before it at 77,115, so the budget has to land between the
    // move being found and the iteration ending. Depth seven reports the new
    // move from about 151,000 nodes on, so anything from there to the 176,633
    // does; this sits between the two with room either side
    let mut s = Session::start(&[]);
    s.say(&format!("position fen {}", SHARP_MIDDLEGAME));
    s.say("go nodes 164000");
